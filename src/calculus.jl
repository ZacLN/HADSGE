...
differentiate(::SymbolParameter{:abs}, args, wrt) = Expr(:call, :ifelse, :($(args[1]) < 0), -differentiate(args[1], wrt), differentiate(args[1], wrt))

differentiate(::SymbolParameter{:>}, args, wrt) = 0
differentiate(::SymbolParameter{:<}, args, wrt) = 0
differentiate(::SymbolParameter{:≤}, args, wrt) = 0
differentiate(::SymbolParameter{:≥}, args, wrt) = 0
...
function simplify(ex::Expr)
    ...
    if all(a -> isa(a, Number), ex.args[2:end]) && length(ex.args) > 1
